...
int main(void)
{
    ...
    fwrite(pg0.r_data, sizeof(pg0.r_data[0]), 8, f1);
    fclose(f1);
    
    pg1 = pg0; 
    
    f1 = fopen("file.bin", "rb");
    size_t r1 = fread(pg1.r_data, sizeof pg1.r_data[0], 8, f1);
    if (r1 == 8)
    {
        printf("Array (register) at %p (slot 0 of page 1) read successfully, contents:\n", (void*)&pg1.r_data);
        puts(pg1.r_data);
    } 
    fclose(f1);
    ...
    */
    ...
    long offset2 = offset_st2 - offset_st0; // 2 * 4096
 ...
 *  3. finally define the structure of metadata page and the actual page and so making use of the header of each.
